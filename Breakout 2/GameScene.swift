...
//  Breakout 2
//
//  Created by Morgan w Rees on 3/27/21.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func makeBricks() {
        ...
        for brick in bricks {
            if brick.parent != nil { brick.removeFromParent()
            }
        }
        ...
        for brick in bricks {
            if contact.bodyA.node == brick ||
                ...
                if brick.color == .blue {
                    ...
                    brick.removeFromParent(); removedBricks += 1
    ...
    override func update(_ currentTime: TimeInterval) {
        if abs(ball.physicsBody!.velocity.dx) < 100 {
            // ball has stalled in x direction, so kick it randomly
